...
namespace MHW_Randomizer
{
    public class GMD
    {
        ...
        public GMD(byte[] GMDBytes)
        {
            //log.Info($"Loading '{path}'");

            MemoryStream ms = new MemoryStream(GMDBytes);
            ...
            long startOfStringBlock = ms.Position;

            for (int i = 0; i < _header.StringCount; i++)
            {
                if (ms.Position == ms.Length)
                {
                    //MessageWindow messageError = new MessageWindow($"Expected to read {_header.StringCount - i} more strings (for a total of {_header.StringCount}). But we already are at the end of the stream!");
            ...
            //MessageWindow message = new MessageWindow("Expected StringBlockSize = " + _header.StringBlockSize);
            //log.Info("Expected StringBlockSize = " + _header.StringBlockSize);
            //message = new MessageWindow("Actual StringBlockSize = " + (ms.Position - startOfStringBlock));
            //log.Info("Actual StringBlockSize = " + (fs.Position - startOfStringBlock));

            if (_header.StringBlockSize != (ms.Position - startOfStringBlock))
            {
                //message = new MessageWindow("Actual StringBlockSize is not the same as the expected StringBlockSize!");
        ...
        /// <summary>
        /// Exports the GMD file to the specified <paramref name="path"/>
        /// </summary>
